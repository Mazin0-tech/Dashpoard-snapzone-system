    ...
    <div class="breadcrumb-wapper style-2">
      <div class="breadcrumb-title-box">
        <h1 class="breadcrumb-title ak-w-70">
          <span class="highlight-text">{{ $contact->breadcrumb_highlight ?? 'Get In Touch' }}</span>
          {{ $contact->breadcrumb_text ?? 'With our Digital Professional Team' }}
        </h1>
...
<section>
  <div class="ak-center">
    <div class="theme-border-wrap contact-form-border hover-animation">
      <!-- Decorative corners omitted for brevity -->
      <div class="container">
        <div class="ak-center">
          <div class="contact-form-box">
            <div class="contact-form-wapper style-2">
              <h5 class="contact-title text-center">{{ $contact->form_title ?? 'Drop a Message' }}</h5>
              <div class="ak-height-30 ak-height-lg-30"></div>
              <form method="POST" action="{{ route('contactfront.index') }}">
                @csrf
                <div class="row g-4">
                  <div class="col-md-6">
                    <input type="text" class="input-text style-2" placeholder="First name" name="firstName" required />
                  </div>
                  <div class="col-md-6">
                    <input type="text" class="input-text style-2" placeholder="Last name" name="lastName" required />
                  </div>
                  <div class="col-md-6">
                    <input type="email" class="input-text style-2" placeholder="Email" name="email" required />
                  </div>
                  <div class="col-md-6">
                    <input type="text" class="input-text style-2" placeholder="Phone" name="phone" required />
                  </div>
                  <div class="col-md-12">
                    <input type="text" class="input-text style-2" placeholder="Subject" name="subject" required />
                  </div>
                  <div class="col-md-12">
                    <textarea name="message" class="textarea-text style-2" rows="5" placeholder="Your Message..."
                      required></textarea>
                  </div>
                </div>
  ...
  <div class="ak-center">
    <div class="contact-cards">
      <div class="contact-card">
        ...
        <p class="contact-text">{!! nl2br(e($contact->address ?? '')) !!}</p>
      </div>
      <div class="contact-card">
        <div class="contact-icon">
          <svg xmlns="http://www.w3.org/2000/svg" width="50" height="51" viewBox="0 0 50 51" fill="none">
      ...
      <div class="contact-card">
        ...
        <p class="contact-text">{!! nl2br(e($contact->email)) !!}</p>
        @endif
        @if(!empty($contact->email2))
        <p class="contact-text">{!! nl2br(e($contact->email2)) !!}</p>
        @endif
        @if(!empty($contact->email3))
        <p class="contact-text">{!! nl2br(e($contact->email3)) !!}</p>
        @endif
      </div>
    </div>
  </div>
...
<div class="ak-google-map ak-bg">
  {!! $contact->iframe ?? '<iframe class="map" src="https://www.google.com/maps/embed?pb=!1m18!...default..."
