                        ...
                        @foreach($blog as $bloggg) 
                            <tr>
                                ...
                                <td>
                                        ...
                                        <img src="{{ asset($bloggg->image) }}" alt="{{ $bloggg->title }}" style="width: 50px; border-radius: 10px; margin: auto;">
                                  
                                </td>
                                <td>{{ $bloggg->title }}</td>
                                <td>{!!$bloggg->description !!}</td>
                                <td>{!!$bloggg->short_description !!}</td>
                                <td>
                                    <a href="{{ route('blog.edit' , $bloggg->id) }}" class="btn btn-primary">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>

                                    <form class="m-auto d-inline" method="post" action="{{ route('blog.destroy',$bloggg->id) }}" onsubmit="return confirm('Are you sure you want to delete this blog post?');">
                                        @csrf
                                        @method('delete')
