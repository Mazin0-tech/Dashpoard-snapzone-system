        ...
        <div class="card custom-card">
            ...
            <div class="card-body">
                <table id="file-export" class="table table-bordered text-nowrap" style="width:100%">
                    ...
                    <tbody>
                        @foreach($sliderproject as $sliderprojec) 
                            <tr>
                                <td>{{ $sliderprojec->id }}</td>
                                <td>{{ $sliderprojec->title }}</td>
                                
                                <td>{{ $sliderprojec->project->title}}</td>
                                <td>
                                
                                        <img src="{{ asset($sliderprojec->image) }}" alt="{{ $sliderprojec->name }}" style="width: 50px; border-radius: 10px; margin: auto;">
                                  
                                </td>
                               
                                <td>
                                    <a href="{{ route('sliderproject.edit' , $sliderprojec->id) }}" class="btn btn-primary">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>

                                    <form class="m-auto d-inline" method="post" action="{{ route('sliderproject.destroy',$sliderprojec->id) }}" onsubmit="return confirm('Are you sure you want to delete this Team post?');">
                                        @csrf
                                        @method('delete')
