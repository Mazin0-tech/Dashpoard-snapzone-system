...
@endpush
        ...
        <div class="mb-3 text-end">
            <a href="{{ route('sliderService.create') }}" class="btn btn-success">
                <i class="fa fa-plus"></i> Create Slider Services
            </a>
        </div>
        ...
        <div class="card custom-card">
            ...
            <div class="card-body">
                <table id="file-export" class="table table-bordered text-nowrap" style="width:100%">
                    ...
                    <tbody>
                        @foreach($sliderService as $sliderServic) 
                            <tr>
                                <td>{{ $sliderServic->id }}</td>
                                <td>{{ $sliderServic->title }}</td>
                                <td>{{ $sliderServic->project->title}}</td>
                                <td>{{ $sliderServic->service->title}}</td>
                                <td>
                                
                                        <img src="{{ asset($sliderServic->image) }}" alt="{{ $sliderServic->name }}" style="width: 50px; border-radius: 10px; margin: auto;">
                                  
                                </td>
                               
                                <td>
                                    <a href="{{ route('sliderService.edit' , $sliderServic->id) }}" class="btn btn-primary">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>

                                    <form class="m-auto d-inline" method="post" action="{{ route('sliderService.destroy',$sliderServic->id) }}" onsubmit="return confirm('Are you sure you want to delete this Team post?');">
                                        @csrf
                                        @method('delete')
